...
def get_sequence_features(texts, max_length=100):
...
class GSpadeModel(nn.Module):
    ...
    def forward(self, batch):
        x = self.node_embeddings(x=batch.x,
...
class GSpadeModelB(nn.Module):
    ...
    def forward(self, batch):
        ...
        # print("sequence_features: ",batch.sequence_features.shape)
        # print("sequence_masks: ",batch.sequence_masks.shape)
        x = self.node_embeddings(x=batch.x,
                                 seqs=batch.sequence_features,
                                 masks=batch.sequence_masks)
        # x = self.input_proj(batch.x)
...
class RelationTagger(nn.Module):
    ...
    def forward(self, enc, edge_index=None, edge_weights=None):
        ...
        # print("field_embeddings: ",self.field_embeddings.shape)
        # print("enc_head: ",enc_head.shape)
...
class GSpadeForIE(nn.Module):
    ...
    def __init__(self, config, embeddings=None):
        ...
        for _ in range(2):
            layer = RelationTagger(config.d_hidden, config.d_head,
                                   config.n_labels)
            self.relations.append(layer)
        self.loss = GSpadeLoss(0.1, 1)
    ...
    def forward(self, batch):
        ...
        #     print("layer(hidden): ",layer(hidden[0]).shape)
        scores = [layer(hidden[0]) for layer in self.relations]
...
def parse_input(texts, bboxes, relations=None, width=None, height=None):
    ...
    n_bboxes = bboxes * 1.0
    n_bboxes[..., 0] = n_bboxes[..., 0] * 1000 / width
    n_bboxes[..., 1] = n_bboxes[..., 1] * 1000 / height
    ret = dict(x=x,
...
class GSpadeNodeEmbedding(nn.Module):
    ...
    def forward(self, x, seqs, masks):
        ...
        for i in torch.sort(masks.unique()).values:
            seq = seqs[masks == i]
            hidden, _ = self.lstm(seq.unsqueeze(1))
...
def get_dense_embedding(text, model, tokenizer):
...
def get_dense_embeddings(texts):
    ...
    embs = [get_dense_embedding(text, model, tokenizer) for text in texts]
