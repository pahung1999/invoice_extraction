...
def get_feature_in_textbox(text, city_list):
    ...
    text_feature = [0] * len(feature_label)
    ...
    text_feature[feature_label.index("date")] = check_case(date_case, text)

    invoice_serial_case = [r"Ký hiệu.*\d", r"Serial.*\d"]
    text_feature[feature_label.index("invoice_serial")] = check_case(
        invoice_serial_case, text)

    invoice_num_case = [r"Số.* H[DĐ] .*\d", r"No. .*\d"]
    text_feature[feature_label.index("invoice_num")] = check_case(
        invoice_num_case, text)

    phone_case = [r"[Đđ]iện thoại.*\d", r"[tT]el .*\d"]
    text_feature[feature_label.index("phone")] = check_case(phone_case, text)

    bank_account_case = [r"Số tài khoản.*\d"]
    text_feature[feature_label.index("bank_account")] = check_case(
        bank_account_case, text)

    bank_name_case = [
        r"[nN]gân hàng .{30}", r"bank.{15}", r"\d .*[nN]gân hàng"
    ]
    text_feature[feature_label.index("bank_name")] = check_case(
        bank_name_case, text)

    tax_case = [r"Mã số thuế.*\d", r"MST.*\d", r"ERC.*\d"]
    text_feature[feature_label.index("tax")] = check_case(tax_case, text)

    city_case = city_list.copy()
    # text_feature[feature_label.index("city")] = check_case(city_case,
    #                                                        text,
    #                                                        check_city=True)

    address_case = city_case + [r"Địa chỉ.{30}"]
    text_feature[feature_label.index("address")] = check_case(
        address_case, text)

    long_num_case = [r"\d{9}"]
    text_feature[feature_label.index("long_num")] = check_case(
        long_num_case, text)

    money_case = [r"\d+,\d+", r"\d+\.\d+"]
    text_feature[feature_label.index("money")] = check_case(money_case, text)
...
class GSpadeModel(nn.Module):
    ...
    def forward(self, batch):
        x = self.node_embeddings(
            ...
            text_features_v2=batch.text_features_v2,
            char_sequence_features=batch.sequence_features,
            char_sequence_masks=batch.sequence_masks,
        )
...
class GSpadeModelB(nn.Module):
    ...
    def forward(self, batch):
        ...
        # print("bboxes.shape.1: ",bboxes.shape)
...
def parse_input(texts, bboxes, relations=None, width=None, height=None):
    ...
    text_features_v2 = torch.tensor(get_text_features_v2(texts))
    # sequence_features, sequence_masks = get_char_sequence_features(texts)
    ...
    # print("bank_ids: ",bank_ids.shape)
    # print("text_features_v2: ",text_features_v2.shape)
    n_bboxes = bboxes * 1.0
    n_bboxes[..., 0] = n_bboxes[..., 0] * 10000 / width
    n_bboxes[..., 1] = n_bboxes[..., 1] * 10000 / height
    ret = dict(x=x,
               bank_ids=bank_ids,
               text_features_v2=text_features_v2,
    ...
    return dict(texts=texts,
                bboxes=bboxes,
                relations=ret.get('relations', None),
                batch=DictInput(ret))
...
class GSpadeNodeEmbedding(nn.Module):
    ...
    def __init__(self, d_seq, d_hidden, n_lstm=1, p_lstm_dropout=0):
        ...
        self.text_feature_embeddings = nn.Embedding(2, d_hidden)
        self.projection = nn.LazyLinear(d_hidden)

    def forward(self, x, bank_ids, text_features_v2, char_sequence_features,
                char_sequence_masks):
        cembeddings = []
        ...
        text_feature_embeddings = self.text_feature_embeddings(
            text_features_v2)
        # seqs = torch.cat(embeddings, dim=0)
        cseqs = torch.cat(cembeddings, dim=0)
        x = torch.cat([x, cseqs, bank_embeddings, text_feature_embeddings],
                      dim=-1)
        x = self.projection(x)
        return x
...
class GSpadeNodeEmbeddingBeta(nn.Module):
    ...
    def __init__(self, d_hidden, n_lstm=1, p_lstm_dropout=0):
        ...
        # self.text_feature_embeddings = nn.Embedding(2, d_hidden // 10)
        self.tfe = nn.ModuleDict()  #text_feature_embedding
        n = len(feature_label)
        for feature in feature_label:
            if feature_label.index(feature) == (n - 1):
                self.tfe[feature] = nn.Embedding(
                    2, d_hidden - (d_hidden // n) * (n - 1))
            else:
                self.tfe[feature] = nn.Embedding(2, d_hidden // n)

    def forward(self, x, sequences, masks, long_masks, text_features_v2):
        sequence_embeddings = []
        for long_mask, sequence in zip(long_masks, sequences):
            if long_mask == 1:
                hidden = self.dlstm(sequence.unsqueeze(1))
            else:
                hidden = self.clstm(sequence.unsqueeze(1))
        ...
        for i in range(n):
            ...
            output_tfe.append(self.tfe[feature_label[i]](
                text_features_v2[i].long()))
        output_tfe = torch.cat(output_tfe, dim=-1)
        ...
        # print("Shape: ")
        # print("x: ",x.shape)
        # print("sequence_embeddings: ",sequence_embeddings.squeeze(1).shape)
        # print("output_tfe: ",output_tfe.shape)
        x = torch.cat([x, sequence_embeddings.squeeze(1), output_tfe], dim=-1)
